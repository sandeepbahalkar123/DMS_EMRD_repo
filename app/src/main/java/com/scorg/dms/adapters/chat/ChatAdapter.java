...
import com.scorg.dms.ui.activities.zoom_images.ZoomImageViewActivity;
...
import static com.scorg.dms.util.DMSConstants.FILE_STATUS.DOWNLOADING;
...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.ListViewHolder> {
    ...
    public void onBindViewHolder(final ListViewHolder holder, final int position) {
        ...
        String timeText = CommonMethods.getFormattedDate(message.getMsgTime(), DMSConstants.DATE_PATTERN.UTC_PATTERN, DMSConstants.DATE_PATTERN.hh_mm_a);
        String dateText = CommonMethods.getDayFromDateTime(message.getMsgTime(), DMSConstants.DATE_PATTERN.UTC_PATTERN, DMSConstants.DATE_PATTERN.DD_MMMM_YYYY, null);
        holder.dateTextView.setText(dateText);

        if (position > 0) {
            String preDate = CommonMethods.getDayFromDateTime(mqttMessages.get(position - 1).getMsgTime(), DMSConstants.DATE_PATTERN.UTC_PATTERN, DMSConstants.DATE_PATTERN.DD_MMMM_YYYY, null);
        ...
        if (message.getSender().equals(DOCTOR)) {
            ...
            if (message.getFileUrl().isEmpty()) {
                ...
                switch (message.getFileType()) {
                    case DMSConstants.FILE.AUD:

                        holder.senderFileLayout.setVisibility(View.VISIBLE);

                        if (message.getUploadStatus() == DMSConstants.FILE_STATUS.UPLOADING) {
                        ...
                        } else if (message.getUploadStatus() == DMSConstants.FILE_STATUS.COMPLETED) {
                            ...
                            holder.senderPhotoLayout.setOnClickListener(new View.OnClickListener() {
                                ...
                                public void onClick(View v) {
                                    if (message.getUploadStatus() == FAILED) {
                                        ...
                                        intent.putExtra(DMSConstants.DOCUMENTS, message.getFileUrl());
                                        intent.putExtra(DMSConstants.IS_URL, isUrl);
                        ...
                        holder.receiverPhotoLayout.setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {
                                if (message.getFileType().equals(LOC)) {
                                    ...
                                    Intent intent = new Intent(mContext, ZoomImageViewActivity.class);
                                    intent.putExtra(DMSConstants.DOCUMENTS, message.getFileUrl());
                                    intent.putExtra(DMSConstants.IS_URL, true);
                                    mContext.startActivity(intent);
                                }
                            }
                        });
